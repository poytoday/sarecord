...
#End Region
Partial Class order_region_sum_detail
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not func.CheckUserPermissions_access("order_region_sum_detail.aspx", Session("User_access")) Then
        ...
        'UserClass.CheckLogin(Page)
        'If Not func.CheckUserPermissions("order_region_sum_detail.aspx", "S") Then
        '    Response.Write("<p>" + "You don't have permissions to access this page" + "<a href=""login.aspx"">&nbsp;" + "Back to login page" + "</a></p>")
        '    Response.End()
        'Else
        '    strConnString = "Server=172.30.203.155;Uid=sa;PASSWORD=1234;database=SA_System;Max Pool Size=400;Connect Timeout=600;"
    ...
    Protected Sub ViewData()
        ...
        End If
        dt.Clear()
        strSQL = "SELECT * FROM sa.order_detail_section WHERE pmcm_id = @pmcm_id"
        dtAdapter = New SqlDataAdapter(strSQL, objConn)
        objCmd = dtAdapter.SelectCommand
        objCmd.Parameters.Add("@pmcm_id", SqlDbType.Int).Value = Request.QueryString("pmcm_id")
        dtAdapter.Fill(dt)
        If dt.Rows.Count > 0 Then
            Me.Label9.Text = dt.Rows(0)("order_section_detail").ToString
        End If
        dt.Clear()
        strSQL = "SELECT pmcm_id, order_region_detail, id_order_type FROM sa.order_detail_region WHERE pmcm_id = @pmcm_id"
        ...
        If dt2.Rows.Count > 0 Then
            ...
            Me.TextBox1.Text = dt2.Rows(0)("order_region_detail").ToString
        End If
        DataBind_gridview1()
        DataBind_gridview2()
    End Sub
    ...
    '    Dim Adpt As New SqlDataAdapter("SELECT * FROM [sa].[order_detail_region] Where 1<>1", StrCon)
    '    Adpt.Fill(Ds, "order_detail_region")
    '    Dt = Ds.Tables("order_detail_region")
    '    Dr = Dt.NewRow
    '    Dr("pmcm_id") = Request.QueryString("pmcm_id")
    '    Dr("order_region_detail") = Me.TextBox1.Text
    '    Dr("id_order_type") = Me.DropDownList1.SelectedValue
    '    Dt.Rows.Add(Dr)
    '    Dim cdb As New SqlCommandBuilder(Adpt)
    '    Adpt.Update(Ds, "order_detail_region")
    '    Ds.AcceptChanges()
    '    Dim sStartPageURL As String = ""
    '    sStartPageURL = "order_region_sum.aspx"
    ...
    Protected Sub LinkButton1_Click1(ByVal sender As Object, ByVal e As System.EventArgs) Handles LinkButton1.Click
        strSQL = "UPDATE sa.order_detail_region SET id_order_type = '" & Me.DropDownList1.SelectedValue & "' WHERE [pmcm_id] = " & Request.QueryString("pmcm_id")
        '& Request.QueryString("pmcm_id")
        objCmd = New SqlCommand(strSQL, objConn)
        objCmd.ExecuteNonQuery()
        strSQL = "UPDATE sa.pmcm_record SET order_id = '" & Me.DropDownList1.SelectedValue & "' WHERE [pmcm_id] = " & Request.QueryString("pmcm_id")
    ...
    Protected Sub LinkButton3_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LinkButton3.Click
        strSQL = "UPDATE sa.order_detail_region SET order_region_detail = '" & Me.TextBox1.Text & "' WHERE [pmcm_id] = " & Request.QueryString("pmcm_id")
        '& Request.QueryString("pmcm_id")
        objCmd = New SqlCommand(strSQL, objConn)
        objCmd.ExecuteNonQuery()
        Me.Label11.Text = "OK"
